...
//static methods
  double COSMCtrlHelper::Longitude2TileX(double Longitude, int nZoom)
  { 
    return (Longitude + 180.0) / 360.0 * pow(2.0, nZoom); 
  }

  double COSMCtrlHelper::Latitude2TileY(double Latitude, int nZoom)
  { 
    return (1.0 - log(tan(Latitude * M_PI/180.0) + 1.0 / cos(Latitude * M_PI/180.0)) / M_PI) / 2.0 * pow(2.0, nZoom); 
  }

  double COSMCtrlHelper::TileX2Longitude(double X, int nZoom)
  {
    return X / pow(2.0, nZoom) * 360.0 - 180;
  }

  double COSMCtrlHelper::TileY2Latitude(double Y, int nZoom)
  {
    double n = M_PI - 2.0 * M_PI * Y / pow(2.0, nZoom);
    return 180.0 / M_PI * atan(0.5 * (exp(n) - exp(-n)));
  }

  //Алгоритм  расчета расстояния "Vincenty's Inverse" (http://en.wikipedia.org/wiki/Vincenty's_formulae). 
  // Реализация на с# http://www.gavaghan.org/blog/2007/08/06/c-gps-receivers-and-geocaching-vincentys-formula/
  double COSMCtrlHelper::DistanceBetweenPoints(const COSMCtrlPosition& position1, const COSMCtrlPosition& position2, double* pdStartBearing, double* pdReverseBearing)
  {
  ...
  COSMCtrlPosition COSMCtrlHelper::GetPosition(const COSMCtrlPosition& position, double dBearing, double dDistance, double* pdEndBearing)
  {
    //First thing to do is setup the constants for the WGS 84 elipsoid
    double a = 6378137.0;
    double b = 6356752.314245;
