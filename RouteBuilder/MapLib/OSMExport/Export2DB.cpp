...
void Export2DB::clearDB()
{
	PGresult *result;
	result = PQexec(mycon, "TRUNCATE TABLE way_tag;");
	result = PQexec(mycon, "TRUNCATE TABLE relations;");
	result = PQexec(mycon, "TRUNCATE TABLE nodes;");
	result = PQexec(mycon, "TRUNCATE TABLE vertices_tmp;");
	result = PQexec(mycon, "DELETE FROM  ways ;");
	result = PQexec(mycon, "DELETE FROM  classes ;");
	result = PQexec(mycon, "DELETE FROM  types ;");
}
void Export2DB::createTables()
{
	...
	string que = "CREATE TABLE ways (gid integer, class_id integer not null, length double precision, name char(200), x1 double precision,";
		   que+= "y1 double precision, x2 double precision,y2 double precision, reverse_cost double precision,rule text, to_cost double precision,";
		   que+= "osm_id integer); SELECT AddGeometryColumn('ways','the_geom',";
		   que+=srid;
		   que+=",'MULTILINESTRING',2);";
		result = PQexec(mycon, que.c_str());
	if (PQresultStatus(result) != PGRES_COMMAND_OK)
        {
...
{
	std::string query = "DROP TABLE IF EXISTS way_tag;";
				query+= "DROP TABLE IF EXISTS relation_ways;";
				query+=	"DROP TABLE IF EXISTS relations;";
				query+=	"DROP TABLE IF EXISTS vertices_tmp;";
...
void Export2DB::exportNode(long long id, double lon, double lat, unsigned short numOfUse )
{
	char tmp_id[20];
	char tmp_lon[15];
	char tmp_lat[15];
...
void Export2DB::exportWay(Way* way)
{
	...
	query+="GeometryFromText('" + way->geom +"', "+srid+")";

	if(way->oneway)
	{
	    query+=", "+ boost::lexical_cast<std::string>(1000000);
	}
	else
	{
...
void Export2DB::createTopology(bool IsCreateSchema)
{
	PGresult *result;
	try
	{
			...
			result = PQexec(mycon,"ALTER TABLE ways ADD COLUMN source integer;");
	...
	catch(...)
	{
		...
		if (PQresultStatus(result) != PGRES_COMMAND_OK)
        {
                std::cerr << "createTopology failed: " 
			<< PQerrorMessage(mycon) 
			<< std::endl;
                PQclear(result);
		}
