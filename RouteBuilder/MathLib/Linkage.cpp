...
{
  tagPOINT MapP;                //результат решени€ системы 
...
tagPOINT CLinkage::SolveModificNewtonMethod(double &De,double &Lo)
{
  ...
  double ApprX=(MapRect.right - MapRect.left)/2.0,ApprY=(MapRect.bottom - MapRect.top)/2.0; //точка приближенна€ к корню
...
tagPOINT CLinkage::SolveDivHalfMethod(double &De,double &Lo)
{
   tagPOINT MapP;
	double x,y;
	bool flag=false;
	double y1=0.0,y2=MapRect.bottom - MapRect.left; //промежуток, на котором ищем корни
