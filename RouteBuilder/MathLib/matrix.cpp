...
{
   CMatrix result(this->cols,this->rows);
   result=(*this);
   for(int i=0;i<n;i++)
     result=(*this)*(*this);
   return result;
...
vector<double> CMatrix::GetEigenValues()
{

  vector<double> koef,values;
  
  if(rows!=cols)
    return values; 
  koef=this->GetCharacteristicPolinomValues();


  return values;

}
vector<double> CMatrix::GetCharacteristicPolinomValues()
{
  vector<double> values,s;
  if(rows!=cols)
    return values; 
  CMatrix *M;
  M=new CMatrix(rows,cols);
  
  *M=(*this);
  
  for(int i=0;i<cols;i++)
  {
	s.push_back((*M).GetSpMatrix());
    *M=(*M)*(*this);
  }
  double p;
  for(int i=0;i<cols;i++)
...
//==========================================================================================================================
void CMatrix::WriteToFile(char* FileName,bool ReWrite)
{
	/*FILE* File;
	//CString g,buf;
	int i,j;
